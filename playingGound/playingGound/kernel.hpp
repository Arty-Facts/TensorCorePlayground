...
template<typename T>
__global__ void matmul_fake_mma_kernel(T* C, T* A, T* B, const unsigned int N) {

    
    int tx = threadIdx.x;
    int ty = threadIdx.y;
    int x = (blockIdx.x * blockDim.x+ tx)* WMMA_C ;
    int y = (blockIdx.y * blockDim.y+ ty)* WMMA_C ;
    ...
    for (int dy = 0; dy < BLOCK_SIZE * WMMA_C; dy += BLOCK_SIZE){
        for (int dx = 0; dx < BLOCK_SIZE * WMMA_C; dx += BLOCK_SIZE){
            tmpSum = (T)0.0f;
            for (k = 0; k < N; k += blockDim.x) {
                __syncthreads();
                sA[ty][tx] = A[(y+dy) * N + k + tx + dx];
                sB[ty][tx] = B[(x+dx) * N + k + ty + dy];
